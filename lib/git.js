...
exports.getGithubUrl = function(options, next) {
  if (!options.template)
    next('Unable to construct a github url. No template provided.', null);
  else if (/^git@github\.com/.test(options.template))
    next(null, options.template);
  else if (~options.template.indexOf('/'))
    next(null, 'git@github.com:' + options.template + '.git');
  else if (options.template && options.user)
    next(null, 'git@github.com:' + options.user + '/' + options.template + '.git');
  else
    next('Unable to construct a github url. The template was not in a known form and no github username was provided.', null);
};

exports.clone = function(options, next, data) {
  var randStr = rand.generate();
  options.tempDir = './' + randStr;
  var clone = cp.spawn('git', ['clone', data.getGithubUrl, randStr], { stdio: 'inherit' });
...
exports.createRemote = function(options, next) {
  var remote = cp.spawn('git', ['remote', 'set-url', 'origin', options.repo.clone_url], { stdio: 'inherit', cwd: options.tempDir });
