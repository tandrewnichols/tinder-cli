...
exports.clone = function(options) {
  return function(next, dependencies) {
    cp.exec('git clone ' + dependencies.getGithubUrl + ' ' + options.repoName, { stdio: 'inherit' }, function(err, stdout, stderr) {
      if (err) next(err);
...
exports.createRemote = function(options) {
  return function(next) {
    ...
    cp.exec('git remote add origin ' + options.repo.clone_url, { stdio: 'inherit' }, function(err, stdout, stderr) {
...
exports.add = function(next) {
  ...
  cp.exec('git add .', { stdio: 'inherit' }, function(err, stdout, stderr) {
    if (err || stderr) next(err || stderr);
    else {
...
exports.commit = function(options) {
  return function(next) {
    ...
    cp.exec('git commit -m "Initial commit using tinder template ' + options.template + '"', { stdio: 'inherit' }, function(err, stdout, stderr) {
      if (err || stderr) next(err || stderr);
      else {
