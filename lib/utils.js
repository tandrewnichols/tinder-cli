...
exports.create = function(options) {
  return {
    ...
    clone: function(next, dependencies) {
      var clone = cp.spawn('git', ['clone', dependencies.getGithubUrl], { stdio: 'inherit' });
      clone.on('close', function(code) {
        if (code) next('git clone ' + dependencies.getGithubUrl + ' returned code ' + code);
        else {
    ...
    copy: function(next, dependencies) {
      var dir = options.clonedDir = _(dependencies.getGithubUrl.split('/')).last().split('.')[0];
