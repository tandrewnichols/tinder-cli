...
exports.create = function(name, template, options) {
  ...
  async.auto({
    ...
    find: ['copy', interpolation.find.bind(interpolation, options)],
    iterate: ['find', interpolation.iterate.bind(interpolation, options)],
    createRepo: git.createRepo.bind(git, options),
    createRemote: ['copy', 'createRepo', git.createRemote.bind(git, options)],
    add: ['iterate', 'createRemote', git.add.bind(git, options)],
