...
exports.cleanup = function(msg, options) {
  ...
  if (options.clean) {
    ...
    async.parallel([
      bash.rm.bind(options.repoName),
      bash.rm.bind(options.tempDir)
    ], function(err) {
      if (err) self.exit(1, err);
      else self.exit(1, 'Removed ' + options.repoName + ' and ' + options.tempDir);
    });
  } else {
    self.exit(1, ('Not removing ' + options.repoName + ' and ' + options.tempDir).red);
  }
};
